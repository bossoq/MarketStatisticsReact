...
} from "https://deno.land/x/supabase@1.1.0/mod.ts";
import type {
  TBMABondYield,
  SETInfoOfficial,
  SETReturnOfficial,
...
const supabaseUrl: string = env.REACT_APP_SUPABASEURL;
const supabaseApiKey: string = env.REACT_APP_SUPABASEAPI;

const supabase = createClient(supabaseUrl, supabaseApiKey);

const updateBondYield = async () => {
...
const updateMarketIndex = async () => {
  ...
  const fetchSETData = async (): Promise<SETInfoOfficial[]> => {
    ...
    const setHTMLDOM: HTMLDocument | null = new DOMParser().parseFromString(
      setHTML,
      "text/html"
    );
    const setTable: Node | undefined = setHTMLDOM
      ?.querySelectorAll("TABLE")
      .item(1);
    const divHTMLDOM: HTMLDocument | null = new DOMParser().parseFromString(
      divYieldHTML,
      "text/html"
    );
    const divTable: Node | undefined = divHTMLDOM
      ?.querySelectorAll("TABLE")
      .item(1);
    if (setTable && divTable) {
      if (
        setTable.children.item(1).children.length ===
        divTable.children.item(0).children.length - 1
      ) {
        for (let i = 0; i < setTable.children.item(1).children.length; i++) {
          const [month, year]: string[] = setTable.children
            .item(1)
            .children.item(i)
            .children.item(0)
            .textContent.split("-");
